...
export default function LiveChart({ asset, strike, currentPrice, showOverlay = true, compact = false }: LiveChartProps) {
  ...
  // TradingView external embed — real BTC/ETH candlestick chart
  useEffect(() => {
  ...
  return (
    <div className="relative w-full rounded-xl overflow-hidden border border-white/10 bg-carbon">
      ...
      {showOverlay && (
        <>
          {/* Strike line — horizontal dashed line with price label */}
          {strike > 0 && (
            <div className="absolute left-0 right-0 top-[30%] pointer-events-none z-10">
