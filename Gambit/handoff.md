...
| **marketId = recycled SLOT ids; module record is one-time registration** | `BinaryMarketsModule.markets(marketId)` holds the slot's FIRST-window registration — its market/pool addresses and expiry do NOT update on window rotation (verified 2026-09-10: every live market shows a past-window record — BY DESIGN, not staleness). NEVER gate creation on module-vs-indexer expiry (that blocked 100% of duels — fixed P0). Live window timing comes from the indexer/price-feed pipeline. Consequence: on-chain `settle()` must NOT trust the module-resolved contract's payouts for rotated windows — `Wager.settle()` refuses (`"stale market record"`), keeping funds escrowed rather than paying an old window's frozen outcome. UI shows the oracle-verified result and explains the escrow. |
